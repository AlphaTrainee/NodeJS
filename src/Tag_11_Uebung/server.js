...
import { db, initDB, Product, seedProducts } from "./database/index.js";
...
// 2. POST: Neues Produkt anlegen
app.post('/api/products', async (req, res) => {
    try {
        // Wir nehmen die Daten aus dem "Body" (den dein api-client.js schickt)
        const newProduct = await Product.create(req.body);
        res.status(201).json(newProduct);
    } catch (error) {
        res.status(400).json({ error: 'Fehler beim Erstellen: ' + error.message });
    }
});

// 3. DELETE: Produkt löschen
...
// 4. PUT: Produkt aktualisieren
app.put('/api/products/:id', async (req, res) => {
    try {
        // 1. Produkt anhand der ID in der URL suchen
        const product = await Product.findByPk(req.params.id);

        if (product) {
            // 2. Produkt mit den Daten aus dem Request-Body aktualisieren
            // Sequelize ist schlau genug, nur die Felder zu ändern, die im Body geschickt wurden
            await product.update(req.body);

            // 3. Das aktualisierte Produkt als JSON zurückgeben
            res.json(product);
        } else {
            // Falls die ID im Baumarkt-Sortiment nicht existiert
            res.status(404).json({ error: 'Produkt nicht gefunden' });
        }
    } catch (error) {
        // Falls z.B. Validierungsfehler auftreten oder die DB streikt
        res.status(500).json({ error: 'Fehler beim Aktualisieren: ' + error.message });
    }
});

app.get('/api/test-error', (req, res, next) => {
    const meinFehler = new Error("Simulation: Da ist was schiefgelaufen!");
    next(meinFehler); // <--- Das schickt den Fehler an deine Middleware
});

app.get('/api/crash', (req, res) => {
    throw new Error("Bumm! Ein Fehler wurde geworfen.");
});
...
async function startServer() {
    try {
        ...
        // 2. Testdaten einfügen (nur wenn DB leer ist)
        await seedProducts();
        
        // 3. Express horchen lassen
...
app.get("/api/users", async (req, res) => {
    try {
        ...
        const users = await User.findAll({ where: whereConditions, order: [['created', 'DESC']] });
